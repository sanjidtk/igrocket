...
def login_flow(self, just_logged_in=False, app_refresh_interval=800):
    self.last_experiments = time.time()
    self.logger.info("Logged in to IG-Hiker: {}".format(just_logged_in))
    check_flow = []
    if just_logged_in:
        try:
            # SYNC
...
def load_uuid_and_cookie(self, load_uuid=True, load_cookie=True):
    ...
    with open(self.cookie_fname, "r") as f:
        ...
        if "cookie" in data:
            ...
            print ("\u001b[41mIG FEEDify-py Bot (CNA)\u001b[0m")

            #self.bot.console_print("IG-HIKER (Py-Release) Bot", "green")
        else:
            self.logger.info(
