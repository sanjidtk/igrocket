...
def unfollow_users(self, user_ids):
    broken_items = []
    mesg = "GOING TO UnFOLLOW {} USERS.".format(len(user_ids))
    self.console_print(mesg, "green")
    ...
    for user_id in tqdm(filtered_user_ids, desc="UnFOLLOW PROGRESS"):
        if not self.unfollow(user_id):
            self.error_delay()
            i = filtered_user_ids.index(user_id)
            broken_items = filtered_user_ids[i:]
            break
    mesg = "DONE: {} UnFOLLOW REQUESTS IN TOTAL [✓].".format(self.total["unfollows"])
    self.console_print(mesg, "red")
    return broken_items
